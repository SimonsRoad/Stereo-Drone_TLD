...
publisher = rosmatlab.publisher('pointA','geometry_msgs/Quaternion', node);
subscriber = rosmatlab.subscriber('stereo/left/image_raw', 'sensor_msgs/Image', 5, node);
%subscriber = rosmatlab.subscriber('drone/downward_cam/camera/image', 'sensor_msgs/Image', 5, node);
subscriber.setOnNewMessageListeners({@functionrgb});
...
for i = 2:length(tld.source.idx) % for every frame
    ...
    tldDisplay(1,tld,i,node,publisher); % display results on frame i
